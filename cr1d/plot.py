...
class MultiColorPatchLine(PatchLine):
	...
	def plot(self, ax, w=0.1, cmap='jet', ec=None, ew=1, vmin=None, vmax=None, th=None):
		self.w  = float(w)
		verts,edges,faces  = self.get_geometry()
		patches = [Polygon(verts[f]) for f in faces]
		coll    = PatchCollection(patches)
		...
		colors = cmap( zn )
		plt.setp(coll, alpha=0.5, facecolor=colors, lw=ew, edgecolor=ec)
		ax.autoscale()
		return patches


def plot_multicolorline(ax, x, y, z=None, cmap='jet', w=0.1, ec=None, ew=1, vmin=None, vmax=None, th=None):
	mcl = MultiColorPatchLine(x, y, z)
	mcl.plot(ax, cmap=cmap, w=w, ec=ec, ew=ew, vmin=vmin, vmax=vmax, th=th)
