...
%=========================================================================
%-------STAGE1 ELLIPSE----------------------------------------------------
function [XY,theta,IJKL,lambdas]=ellipse(x,y,p) %calculates ellipse
...
%modified from: Duarte M, Zatsiorsky VM. Exp Brain Res 2002;146(1):60-9.
...
XYcentered=k*[cos(c),sin(c)]*sqrt(lambdas)*IJKL; %ellipse around [0,0]
...
CI1x=NaN(r,1); CI1y=NaN(r,1); CI2x=NaN(r,1); CI2y=NaN(r,1);
    for t=1:r 
[~,thetas,IJKL,lambdas]=ellipse(x(t,:),y(t,:),p);% retrieve ellipse analysis
